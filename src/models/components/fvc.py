...
from src.models.components.FVC_subnet import (
    FeatureDecoder,
    FeatureEncoder,
    ICLR17EntropyCoder,
    NIPS18nocEntropyCoder,
    OffsetPriorDecodeNet,
    OffsetPriorEncodeNet,
    PCD_Align,
    ResDecodeNet,
    ResEncodeNet,
    ResPriorDecodeNet,
...
def save_model(model, optimizer, iter):
    torch.save(model.state_dict(), f"./snapshot/iter{iter}.model")
    checkpoint = {
...
class FVC(nn.Module):
    def __init__(
        ...
        self.aligner = PCD_Align(out_channel_mv=out_channel_mv)

        self.OffsetPriorEncoder = OffsetPriorEncodeNet(out_channel_mv=out_channel_mv)
        self.OffsetPriorDecoder = OffsetPriorDecodeNet(out_channel_mv=out_channel_mv)
        ...
        self.resEncoder = ResEncodeNet(
            out_channel_M=out_channel_M, out_channel_resN=out_channel_resN
        )
        self.resDecoder = ResDecodeNet(
            out_channel_M=out_channel_M,
            out_channel_resN=out_channel_resN,
            out_channel_resM=out_channel_resM,
        )
        self.resPriorEncoder = ResPriorEncodeNet(out_channel_mv=out_channel_mv)
        self.resPriorDecoder = ResPriorDecodeNet(out_channel_mv=out_channel_mv)
    ...
    def Q(self, x):
        if self.training:
            return x + nn.init.uniform_(torch.zeros_like(x), -0.5, 0.5)
        else:
    ...
    def AddResidual(self, input_feature, aligned_feature):
        ...
        q_encoded_residual_prior, bits_residualz = self.EntropyCoding_residualz(
            encoded_residual_prior
        )
        decoded_residual_prior = self.resPriorDecoder(q_encoded_residual_prior)

        q_encoded_residual, bits_residualf = self.EntropyCoding_residualf(
            encoded_residual, decoded_residual_prior
        )
    ...
    def GetLoss(
        ...
        out["bpp"] = (
            out["bpp_offsetf"] + out["bpp_offsetz"] + out["bpp_residualf"] + out["bpp_residualz"]
        )
        # out["rd_loss"] = (
        #     self.true_lambda
        #     * (
        #         self.finalmse * out["mse_loss"]
        #         + self.motionmse * out["align_loss"]
        #         + 0.1 * out["input_loss"]
        #     )
    ...
    def forward(self, ref_image, enhanced_input_image, input_image):
        # from pixel space to feature space
        ref_feature = self.Encoder(ref_image)
        input_feature = self.Encoder(enhanced_input_image)

        input_recon = self.Decoder(input_feature)

        # feature space deformable compensation
        aligned_feature, bits_offsetf, bits_offsetz = self.AlignMultiFeature(
            ref_feature, input_feature
        )

        # feature space residual compression
        recon_feature, bits_residualf, bits_residualz = self.AddResidual(
            input_feature, aligned_feature
        )
        ...
        if not self.training:
            ...
            align_image = align_image.clamp(0.0, 1.0)
