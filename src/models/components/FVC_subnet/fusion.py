...
class Nonlocal_Fusion(nn.Module):
    ...
    def forward(self, aligned_fea):
        ...
        emb = self.tAtt_1(aligned_fea.view(-1, C, H, W)).view(
            B, N, -1, H, W
        )  # [B, N, C2(nf), H, W]
        ...
        for i in range(N):
            ...
            emb_nbr = F.conv2d(emb_nbrp, self.nonlocal_kernel, padding=p // 2, groups=C2).view(
                B, C2, p * p, H, W
            )
        ...
        cor_prob = (
            cor_prob.view(B, N, -1, H, W).unsqueeze(2).repeat(1, 1, C2, 1, 1, 1).view(B, -1, H, W)
        )  # B, N*C*p*p, H, W

        aligned_fea = F.conv2d(
            aligned_fea.view(B * N, C2, H, W), self.nonlocal_kernel, padding=p // 2, groups=C2
        )  # B*N, C*p*p, H, W
