...
from .basics import MaskedConvolution2D, Q, Resblocks
...
class Context_prediction_net(nn.Module):
    ...
    def __init__(self, out_channel_resM=128):
        super().__init__()
...
class NIPS18EntropyCoder(nn.Module):
    ...
    def __init__(self, out_channel_resM=128):
        super().__init__()
        self.context_model = Context_prediction_net(out_channel_resM)
...
class NIPS18nocEntropyCoder_adaptive(nn.Module):
    ...
    def __init__(self, out_channel_mv=128):
        super().__init__()
        self.parameters_net = Parameter_net()
        self.maskprediction = MaskPredictionNet(out_channel_mv)
    ...
    def getbits(self, x, musigma, mask):
        ...
        if self.training:
            bits = torch.sum(
                torch.clamp(-1.0 * torch.log(probs + 1e-5) / math.log(2.0), 0, 50) * mask
            )
        else:
    ...
    def forward(self, x, musigma):
        ...
        shape11, shape12, shape21, shape22, shape24, shape42, shape44 = (
            q_x11.shape[2:4],
            q_x12.shape[2:4],
            q_x21.shape[2:4],
            q_x22.shape[2:4],
        ...
        mask11, mask12, mask21, mask22, mask24, mask42, mask44 = (
            F.interpolate(mask11, shape11),
            F.interpolate(mask12, shape12),
            F.interpolate(mask21, shape21),
            F.interpolate(mask22, shape22),
            F.interpolate(mask24, shape24),
            F.interpolate(mask42, shape42),
            F.interpolate(mask44, shape44),
        )
        ...
        q_block_x = (
            F.interpolate(q_x11 * mask11, shape11)
            + F.interpolate(q_x12 * mask12, shape11)
            + F.interpolate(q_x21 * mask21, shape11)
            + F.interpolate(q_x22 * mask22, shape11)
            + F.interpolate(q_x24 * mask24, shape11)
            + F.interpolate(q_x42 * mask42, shape11)
            + F.interpolate(q_x44 * mask44, shape11)
        )
...
class NIPS18nocEntropyCoder_ignore(nn.Module):
    ...
    def __init__(self, out_channel_mv=128):
        super().__init__()
        self.parameters_net = Parameter_net()
        self.ignore_net = IgnorePrediction(out_channel_mv)
    ...
    def forward(self, x, musigma):
        ...
        if self.training:
            ...
            total_bits = torch.sum(
                torch.clamp(-1.0 * torch.log(probs) / math.log(2.0), 0, 50) * (ignore.detach())
            )
