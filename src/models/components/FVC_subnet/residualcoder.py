...
from .basics import Resblocks


class ResEncodeNet(nn.Module):
    """Residual Encode Net."""

    def __init__(self, out_channel_M=64, out_channel_resN=128):
        super().__init__()
...
class ResDecodeNet(nn.Module):
    ...
    def __init__(self, out_channel_M=64, out_channel_resN=128, out_channel_resM=128):
        super().__init__()
        self.deconv1 = nn.ConvTranspose2d(
            out_channel_resM, out_channel_resN, 5, stride=2, padding=2, output_padding=1
        )
        self.res1 = Resblocks()
        self.deconv2 = nn.ConvTranspose2d(
            out_channel_resN, out_channel_resN, 5, stride=2, padding=2, output_padding=1
        )
        self.res2 = Resblocks()
        self.deconv3 = nn.ConvTranspose2d(
            out_channel_resN, out_channel_M, 5, stride=2, padding=2, output_padding=1
        )
...
class ResPriorDecodeNet(nn.Module):
    ...
    def __init__(self, out_channel_mv=128):
        super().__init__()
        self.deconv1 = nn.ConvTranspose2d(
            out_channel_mv, out_channel_mv, 5, stride=2, padding=2, output_padding=1
        )
        self.relu1 = nn.LeakyReLU(negative_slope=0.2)
        self.deconv2 = nn.ConvTranspose2d(
            out_channel_mv, out_channel_mv * 3 // 2, 5, stride=2, padding=2, output_padding=1
        )
        self.relu2 = nn.LeakyReLU(negative_slope=0.2)
        self.deconv3 = nn.ConvTranspose2d(
            out_channel_mv * 3 // 2, out_channel_mv * 2, 3, stride=1, padding=1
        )
