...
class EncodeDecodeIntra(nn.Module):
    ...
    def _rate_proxy_gaussian(self, inputs: torch.Tensor, axis: List[int]) -> torch.Tensor:
        ...
        """
        assert inputs.dim() >= np.max(np.abs(axis)) + 1
        ...
        return (
            torch.sum(hires_entropy.view(inputs.shape[0], -1), dim=1)
            * torch.prod(torch.tensor(inputs.shape[1:], dtype=torch.float32))
            / np.log(2)
        )
    ...
    def _rate_proxy_jpeg(
        ...
        def calculate_non_zeros(
            ...
            for k in dct_coeffs:
                num_nonzeros += torch.sum(
                    (torch.log(1 + torch.abs(dct_coeffs[k] / qstep) + 1e-9)).view(
        ...
        def encode_decode_inputs_with_jpeg() -> Tuple[torch.Tensor, torch.Tensor]:
            ...
            jpeg_decoded = torch.tensor(jpeg_decoded, dtype=torch.float32).to(
                three_channel_inputs.device
            )
            jpeg_rate = torch.tensor(jpeg_rate, dtype=torch.float32).to(
                three_channel_inputs.device
            )
        ...
        nonzero_times_rate = torch.mul(num_nonzero_dct_coeffs, jpeg_rate)
        nonzero_times_nonzero = torch.mul(num_nonzero_dct_coeffs, num_nonzero_dct_coeffs)
        ...
        # the optimizer.
        line_weights = torch.div(nonzero_times_rate, nonzero_times_nonzero + 1).detach()

        return torch.mul(num_nonzero_dct_coeffs, line_weights)
